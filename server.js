...
function gateKeeper(req, res, next) {
  ...
  let userQueryString = req.get('x-username-and-password');
  
  //user=ellen&pass=superSecretPassword
  //queryString.parse('catName=george&dogName=georgette')
 let userObject = queryString.parse(userQueryString);
  //userObject = {user:'Joeschmoe@business.com', pass:'password'}
  
  function userFind(element){
    if  (element.userName === userObject.user && element.password === userObject.pass)
  return true;
  
  else return false}
 
  let foundUser =  USERS.find(userFind);
  req.user = foundUser;
  next();
}
